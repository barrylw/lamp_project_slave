...
RN8209C_PARAM rn8209c_papameter = 
{
      ...
      .Ku           = 1188111,                 //电压系数   mv                              
      .KIa          = 100500,                 //电流A系数  ma
      .Kp           = 23.59,               //功率系数   uW ,手动计算结果，若修改参数，必须手动修改
      .ChkSum       = 0,                 //校验和
      .calibration  = 1,                 //电表是否校准
      .EC           = 6400,
      .pwmValue     = 100,
      .lightTime    = 0,
 };
...
#endif 
   if (rn8209c_papameter.calibration == 1) //不校表的话不保存校表数据
   {
       if (flash_ok == true)
       {
        #ifdef PRINTF_DEBUG
...
void read_UIP(void)
{
    ...
    currentUIP.Uv  =   ((temp & 0x800000) > 0) ? 0:(u16)(220.0*10/rn8209c_papameter.Ku*temp); //以 1/10V为单位
    ...
    currentUIP.Pa = (u16)(rn8209c_papameter.Kp*temp/1000000.0);//本身计算出来的结果就是扩大了10^6，以uW为单位，现在以W为单位
}
